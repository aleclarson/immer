...
export const hooks = Object.seal({
    /**
     * Called in the "finalize" phase for every property changed by a producer,
...
export function setHook(key, hook) {
    if (typeof hook !== "function") throw TypeError("Expected a function")
    if (key in hooks) hooks[key] = hook
    else throw Error("Non-existent hook cannot be set: " + key)
}
...
function finalizeObject(copy, state, path, patches, inversePatches) {
    ...
    each(copy, (prop, value) => {
        if (value !== base[prop]) {
            ...
            if (hooks.setProperty) {
                hooks.setProperty(state, prop, value)
            }
        }
    })
